...
namespace pdj.tiny7z.Compression.PPMd.H
{
    internal class PPMContext : Pointer
    {
        ...
        internal void rescale(ModelPPM model)
        {
            ...
            p.Freq = Util.URShift((p.Freq + Adder), 1);
            ...
            if (p.Freq == 0)
            {
                ...
                if (NumStats == 1)
                {
                    ...
                    {
                        // tmp.Freq-=(tmp.Freq >> 1)
                        tmp.DecrementFreq(Util.URShift(tmp.Freq, 1));
                        EscFreq = Util.URShift(EscFreq, 1);
                    }
                    while (EscFreq > 1);
                    model.SubAlloc.freeUnits(freqData.GetStats(), Util.URShift((OldNS + 1), 1));
            ...
            int n0 = Util.URShift((OldNS + 1), 1), n1 = Util.URShift((NumStats + 1), 1);
            if (n0 != n1)
            {
        ...
        internal int getArrayIndex(ModelPPM Model, State rs)
        {
            ...
            ret += ((Util.URShift(Model.RunLength, 26)) & 0x20);
            return ret;
        }

        internal int getMean(int summ, int shift, int round)
        {
            return (Util.URShift((summ + (1 << (shift - round))), (shift)));
        }
        ...
        internal void decodeBinSymbol(ModelPPM model)
        {
            ...
            else
            {
                ...
                model.InitEsc = ExpEscape[Util.URShift(bs, 10)];
                model.NumMasked = 1;
