...
using Decoder = pdj.tiny7z.Compression.LZMA.RangeCoder.Decoder;
...
namespace pdj.tiny7z.Compression.PPMd.H
{
    internal class ModelPPM
    {
        ...
        {
            for (int i = 0; i < charMask.Length; ++i) charMask[i] = 0;
            ...
            for (pc.Address = maxContext.Address; pc.Address != minContext.Address; pc.Address = pc.getSuffix())
            {
                if ((ns1 = pc.NumStats) != 1)
                {
                    if ((ns1 & 1) == 0)
                    {
                        ...
                        pc.FreqData.SetStats(SubAlloc.expandUnits(pc.FreqData.GetStats(), Util.URShift(ns1, 1)));
        ...
        public int decodeChar(Decoder decoder)
        {
            ...
            else
            {
                ...
                initEsc = PPMContext.ExpEscape[Util.URShift(bs, 10)];
                int i;
                for (i = 0; i < 256; i++)
                {
                    charMask[i] = -1;
